<template>
    <div>
        <LuxHeader>
            ...
            <p slot="center" class="title">注册第五大道</p>
        </LuxHeader>
        <div class="msg">
            ...
            <LoginInput class="input" placeholder="请输入密码" @change="pwd = $event"></LoginInput>
            <div class="loading">
                <Loading v-if="loginFlag"></Loading>
            </div>
            <LoginButton class="register" text="注册" :handler="register"></LoginButton>
        </div>
...
<script type="text/ecmascript-6">
	...
	export default {
		...
		methods:{
			register(){
				this.loginFlag = true;
				axios.post('http://localhost:3000/login/register',{phone:this.phone,code:this.code,pwd:this.pwd})
					.then(result => {
						console.log(result);
						if(result.data === 1)
